...
5단계 워크플로우의 마지막 단계. 사람이 명시적으로 진행/보류/반려를 선택하고 근거를 같이 박는다. 채팅으로 "그럼 진행해주세요" 하고 끝내면 그 이유가 같이 사라지므로, 이 단계의 핵심은 의사결정을 휘발되지 않게 만드는 것이다.
...
`/changeset` 호출은 자동화하지 않는다 (R4) — 사용자가 직접 명령을 입력해야 다음 단계로 넘어가는 휴식 지점을 보존한다.
