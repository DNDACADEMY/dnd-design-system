...
### 5-5. 막히는 지점 발견 시 (피드백 루프)

단계 스킬을 실행하다가 절차가 부족하거나 룰이 모호하다고 느끼면 **그 자리에서 즉시 업데이트하지 않는다** — 디자이너/사용자의 흐름을 끊지 않기 위함이다. 대신 막힌 지점을 짧게 기록하고(어느 단계, 어떤 입력에서, 무엇이 부족했는지 한두 줄) 워크플로우는 그대로 진행한다.

세션이 끝나거나 결정 단계까지 도달한 뒤, `workflow-admin` 을 호출해 누적된 메모로 SSOT 와 영향 스킬을 한 번에 갱신한다. 같은 지점에서 다음 사람이 다시 막히지 않게 하는 게 목적이다.

기록 위치 권장:

- 진행 중인 변경에서 발견했다면 `.claude/workflow/<id>/notes.md` (gitignore 됨)
- 워크플로우 자체의 일반 이슈면 채팅에서 "메모: ..." 로 짧게 — 세션 종료 시 모은다

`workflow-admin` 자신의 절차에 문제가 있으면 그 SKILL.md 는 사람이 직접 수정한다 (자기 참조 루프 회피).
...
- 2026-05-07: 거버넌스 §5-5 추가 — 막히는 지점 피드백 루프. 단계 스킬에 "절차 개선" 안내 박음.
