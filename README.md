...
DND Academy 디자인 시스템 모노레포. UI 컴포넌트(`@dnd-lab/desktop`)와 디자인 토큰(`@dnd-lab/token`)을 한 곳에서 빌드·배포한다.
...
pnpm --filter @dnd-lab/desktop storybook      # Storybook 개발 서버 (포트 6006)
pnpm --filter @dnd-lab/desktop build-storybook
pnpm --filter @dnd-lab/desktop test-storybook # 인터랙션/접근성 테스트
```

### 4. 컴포넌트 추가

```bash
pnpm --filter @dnd-lab/desktop generate:component
```
...
```bash
# 의존성 추가/업데이트
pnpm --filter @dnd-lab/desktop add <pkg>
pnpm install
...
- **Vanilla Extract**: 토큰은 `@dnd-lab/token` 의 `primitive` / `semantic` 만 사용
